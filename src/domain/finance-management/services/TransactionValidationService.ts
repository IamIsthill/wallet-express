...
export class TransanctionValidationService {
  public static validateExpenseAmount(
    account: Account,
    ...
    if (
      ...
      if (transaction.amount > account.balance) {
        throw new Error(
          `Expense amount (${transaction.amount}) exceeds account balance (${account.balance})`,
        );
      }
    }
  }

  public static validateTransferAmount(
    account: Account,
    transaction: Transaction,
  ): void {
    ...
    if (account.balance < transaction.amount) {
      throw new Error(
        "Cannot perform transaction as the account has insufficient balance",
      );
    }
  }
}
