...
describe("GetAccountService", () => {
  ...
  it("should find an existing account and return a dto on success", async () => {
    ...
    (mockAccountRepository.getAccountByAccountId as Mock).mockResolvedValue(
      mockAccount,
    );

    const res = await getAccountService.use(getAccountDto);

    expect(mockAccountRepository.getAccountByAccountId).toHaveBeenCalledWith(
      "account-id",
    );
  ...
  it("should throw a ServiceError wrapping the DatabaseError from the repository", async () => {
    ...
    (mockAccountRepository.getAccountByAccountId as Mock).mockRejectedValue(
      databaseError,
    );
    ...
    await expect(getAccountService.use(getAccountDto)).rejects.toHaveProperty(
      "cause",
      databaseError,
    );
  });
